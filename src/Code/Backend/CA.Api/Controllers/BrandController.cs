...
namespace CA.Api.Controllers
{
    ...
    public class BrandController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ApiResponse<MetaData<ShapedEntityDTO>>> Get([FromQuery] GetAllBrandParameter filter)
        {
            var _response = await _mediator.Send(new GetAllBrandQuery() { PageSize = filter.PageSize, PageNumber = filter.PageNumber, Fields = filter.Fields, OrderBy = filter.OrderBy, Search = filter.Search, Route = Request.Path.Value });
