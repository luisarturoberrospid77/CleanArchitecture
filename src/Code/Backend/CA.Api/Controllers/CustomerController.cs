...
namespace CA.Api.Controllers
{
    ...
    public class CustomerController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ApiResponse<MetaData<ShapedEntityDTO>>> Get([FromQuery] GetAllCustomerParameter filter)
        {
            var _response = await _mediator.Send(new GetAllCustomerQuery() { PageSize = filter.PageSize, PageNumber = filter.PageNumber, Fields = filter.Fields, OrderBy = filter.OrderBy, Search = filter.Search, Route = Request.Path.Value });
