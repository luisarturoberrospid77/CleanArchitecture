...
namespace CA.Api.Controllers
{
    ...
    public class SupplierController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ApiResponse<MetaData<ShapedEntityDTO>>> Get([FromQuery] GetAllSupplierParameter filter)
        {
            var _response = await _mediator.Send(new GetAllSupplierQuery() { PageSize = filter.PageSize, PageNumber = filter.PageNumber, Fields = filter.Fields, OrderBy = filter.OrderBy, Search = filter.Search, Route = Request.Path.Value });
