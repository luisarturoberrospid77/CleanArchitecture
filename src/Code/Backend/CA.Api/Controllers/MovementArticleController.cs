...
namespace CA.Api.Controllers
{
    ...
    public class MovementArticleController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ApiResponse<MetaData<ShapedEntityDTO>>> Get([FromQuery] GetAllMovementArticleParameter filter)
        {
            var _response = await _mediator.Send(new GetAllMovementArticleQuery() { PageSize = filter.PageSize, PageNumber = filter.PageNumber, Fields = filter.Fields, OrderBy = filter.OrderBy, Search = filter.Search, Route = Request.Path.Value });
