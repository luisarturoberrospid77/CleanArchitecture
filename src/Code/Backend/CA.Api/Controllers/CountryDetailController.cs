...
namespace CA.Api.Controllers
{
    ...
    public class CountryDetailController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ApiResponse<MetaData<ShapedEntityDTO>>> Get([FromQuery] GetAllCountryDetailParameter filter)
        {
            var _response = await _mediator.Send(new GetAllCountryDetailQuery() { PageSize = filter.PageSize, PageNumber = filter.PageNumber, Fields = filter.Fields, OrderBy = filter.OrderBy, Search = filter.Search, Route = Request.Path.Value });
